...
//    LPC_SC->PCONP |= (1 << 21);
    ...
    /* Set SSEL0 as GPIO, output high */
		...
		// В оригинале к CS карты подключен SSEL. У меня хз почему всесто SSEL подключен P0.5
		// Будем дрыгать им вручную...  
		PINSEL0 &= ~(1 << 11);
    ...
    /* Configure SSP0_PCLK to CCLK(100MHz), default value is CCLK/4 */
		/* Configure SSP1_PCLK to CCLK(72MHz), default value is CCLK/3 */
