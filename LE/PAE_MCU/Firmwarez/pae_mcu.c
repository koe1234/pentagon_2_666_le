...
{ led0_on();
	...
	if(bootloader) start_bootloader();
	if(condition_1) 
	{
		cycle_counter++;
		if (klava) { klava = false; upload_data(); }  
		if(reset)	{	reset = false; z80_reset();	}
		if(received_status == 0) { get_cmos_status(); cmos_counter = 0; cmos_ready = false; } 
		else 
		if(cmos_counter > 64) { get_cmos_data(received_status); received_status = 0; }
		if(cmos_ready) {fpga_send_command(31); while(send_command_flag != 0xff) {}; fpga_send_command(0); while(send_command_flag != 0xff) {}; cmos_ready = false;}  
	}
  if(condition_2) 
	{
		cycle_counter++;
	...
	if(condition_5) { cycle_counter++; fpga_data_transmit_flag = 0; get_fpga_data(); } 
...
unsigned char index;
	...
	get_cmos_status_flag = true;
	get_fpga_data_flag = true;


	fpga_data_transmit_flag = 0xff;
	...
	mouse = mouse_setup();
	//mouse = 1;
	if(mouse > 0) {m_b = 7; m_x = 127; m_y = 127; } else {m_b = 0xff; m_x = 0xff; m_y = 0xff;};
...
///////////////////////////////// command/data transfer functions //////////////////////////
void get_fpga_data(void)
{

	I21CONSET = (1<<6);      					
	i2c_command = (17<<1); 
	get_fpga_data_flag = false;
	i2c_byte_number = 0;
	I21CONCLR = 0x000000FF;
	I21CONSET = 0x00000040;
	I21CONSET = 0x00000020;
	while (fpga_data_transmit_flag == 0) {}; 
	fpga_send_command(0);
...
unsigned long bf;
	...
	for(index = 0; index < 64; index++)
	{
		...
		cmos_a++;	//=cmos_a+1;
		if ((status >> 1) & 1 ) 
			RTC_SEC = ((buffer4cmos[0] & 0xf0) >> 4)*10 + (buffer4cmos[0] & 0x0f);
		...
		//RTC_SEC = ((buffer4cmos[0] & 0xf0) >> 4)*10 + (buffer4cmos[0] & 0x0f);
		//RTC_MIN = ((buffer4cmos[2] & 0xf0) >> 4)*10 + (buffer4cmos[2] & 0x0f);
		//RTC_HOUR = ((buffer4cmos[4] & 0xf0) >> 4)*10 + (buffer4cmos[4] & 0x0f);
		//RTC_DOW = buffer4cmos[6] - 1;
		//RTC_DOM = ((buffer4cmos[7] & 0xf0) >> 4)*10 + (buffer4cmos[7] & 0x0f);
		//RTC_MONTH = ((buffer4cmos[8] & 0xf0) >> 4)*10 + (buffer4cmos[8] & 0x0f);	
		//RTC_YEAR = 2000 + ((buffer4cmos[9] & 0xf0) >> 4)*10 + (buffer4cmos[9] & 0x0f);
...
{
	I21CONSET = 1 << 6;
	i2c_command = 9 << 1; // command '9' 
	get_cmos_status_flag = false;
	I21CONCLR = 0x000000FF;
	I21CONSET = 0x00000040;
	I21CONSET = 0x00000020;
}
...
unsigned char index;
	...
	i2c_buffer[8] = RTC_DOW+1; //((((RTC_DOW + 1)/10) & 0x0f) << 4) | ((RTC_DOW + 1)%10);
...
//if(!screen_from_sd("firmware/logo.rom")) { data_base_addr = logo; rom_from_rom(5, logo_size); }
//if(!rom_from_sd(64 + 8,"firmware/gluk.rom")) { data_base_addr = basic48; rom_from_rom(64 + 8, basic48_size); }
//if(!rom_from_sd(64 + 10,"firmware/basic128.rom"))	{ data_base_addr = basic48; rom_from_rom(64 + 10, basic48_size); }
//if(!rom_from_sd(64 + 9,"firmware/trdos.rom"))	{ data_base_addr = basic48; rom_from_rom(64 + 9, basic48_size); }
//if(!rom_from_sd(64 + 11,"firmware/basic48.rom")) { data_base_addr = basic48; rom_from_rom(64 + 11, basic48_size); }
//if(!rom_from_sd(64 + 12,"firmware/fatall.rom"))	{data_base_addr = basic48; rom_from_rom(64 + 12, basic48_size); }
...
unsigned int read, index;
	...
	//if(f_opendir(&directory,"firmware") != FR_OK) return 0;
...
//	k_int_flag = 1;	
	...
	//if(clk_status_in != clk_status_out) 
	//{led0_on(); }
	//else led0_off();
...
//	led0_on(); led0_off();
	...
	//if((scan_code_number > 0) && !klava) translate(); 
	...
	//if(~(FIO1PIN >> 27) & 1) led0_on(); else led0_off();
...
void i2c_irq(void) __irq
{
	switch (I21STAT)
	{
		case 0x08: // start condition has been sent
			...
			{
				I21DAT = i2c_command & ~1; // transmit slave address => command
				if(!send_command_flag) send_command_flag++;
				if(!send_data_flag) send_data_flag++;
				break;
			}
			if(!get_cmos_status_flag || !get_fpga_data_flag)
				I21DAT = i2c_command | 1;
			break;
		case 0x18: // slave address + W (0 in first byte) has been sent
			if(send_command_flag == 1)
				{
					send_command_flag = 0xff; I21CONSET = 0x10;
				}
			if(send_data_flag == 1)
			{
			...
			//if(!get_cmos_status_flag || !get_fpga_data_flag) I21CONSET = 0x20; // repeat start
			break;
		case 0x28: // data has been transmitted, ack received 
			if(send_data_flag == 2) 
				{
					if(i2c_byte_number <= i2c_number_of_bytes) 
						{
							I21DAT = i2c_buffer[i2c_byte_number];
							i2c_byte_number++;
		...
		case 0x10: // repeated start has been sent
			...
			if(!get_cmos_status_flag || !get_fpga_data_flag)
				{
					if(!get_cmos_status_flag) I21DAT = 25; else I21DAT = (i2c_command | 1); 
				}
			break;
		case 0x40: // slave address + r was transmitted. ack has been received
			if(!get_cmos_status_flag) I21CONCLR = 4; // clear AAC flag. nack will be transmitted
			if(!get_fpga_data_flag) I21CONSET = 4; // set AAC flag. ack will be transmitted
		break;
		...
		case 0x58: // data has been received, nack has been transmitted
			if(!get_cmos_status_flag)
				{
					received_status = I21DAT;
					get_cmos_status_flag = true;
				}
			if(!get_fpga_data_flag)
				if(!fpga_data_transmit_flag)	
						{
							if(fpga_data_array[15] & 1) 
								{
									buffer4cmos[(fpga_data_array[15]>>2) & 0x3F] = I21DAT;
									cmos_ready = true;
								}		
							get_fpga_data_flag = true;
							fpga_data_transmit_flag = 0xff;
		...
		case 0x50: // data has been received, ack has been transmitted
			if(!fpga_data_transmit_flag)
			{	
				fpga_data_array[i2c_byte_number] = I21DAT;   
				if(i2c_byte_number < 16)
					I21CONSET = 4; // set AAC flag. ack will be transmitted
				else
					I21CONCLR = 4; // clear AAC flag. nack will be transmitted
				i2c_byte_number++;	
			}	
			break;
		default:
...
void start_bootloader(void)
{
	...
	user_code_entry = (void (*)(void))0x00000000;
	user_code_entry();
}
